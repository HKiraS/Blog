...
export function RecentPosts({ data }) {
  ...
  return (
    <section className="">
      <div className="max-w-screen-2xl mx-auto recent-posts">
        <h1 className="title-b-desktop w-fit relative my-16 max-md:justify-self-center">
          Destaques
        </h1>
        <div className="posts-container container max-md:px-4 mx-auto grid grid-cols-10 max-md:flex max-md:flex-col max-lg:gap-0 px-4 max-lg:px-0">
          <div className="col-span-5 row-span-3 w-full h-full flex flex-col gap-8 ">
            ...
            <div className="small-posts-container col-span-5 row-start-2 row-end-4 max-w-full grid grid-cols-2 gap-8 max-md:hidden">
          ...
          <div className="side-posts-container col-start-7 col-end-11 row-span-full grid grid-cols-1 place-content-center gap-12 max-md:gap-0 ">
            {sidePosts.map(
              ({ img, title, description, timeRead, date, classes }, index, arr) => {
                return (
                  <div key={index}>
                    <Posts
                      img={img}
                      ...
                      classes={classes}
                    />
                    {arr.length - 1 !== index ? <span className="separator md:hidden max-md:h-0.5 max-md:w-5/6 max-md:my-8 place-self-center opacity-20"></span> : null}
                  </div>
                );
              },
            )}
